...
module.exports = fp(async function (fastify, opts) {
  ...
  setTimeout(async function updateInteractions() {
    ...
    for (const outdatedContract of contractsToBeUpdated) {
      let txs = (await fetchTxsFromHeight(fastify, outdatedContract.contractId, outdatedContract.lastUpdateBlock)).map(t => ({ ...t, contract: outdatedContract.contractId, id: "interactions:`" + t.id + "`" }))
...
async function fetchTxsFromHeight(fastify, contractId, height) {
  height -= 1
  ...
  while (pageInfo.hasNextPage) {
    ...
    let gqlreply = await fastify.fetch(config.arweaveGQL, txQuery(height, [["Contract", contractId]], cursor)).then(res => res.json()).catch(e => pageInfo.hasNextPage = false)
